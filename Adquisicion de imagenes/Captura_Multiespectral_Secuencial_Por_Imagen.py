...
#################################
# PUERTO SERIAL si es 0 buscara los puertos y le dara la opcion de escoger el puerto serial en caso de la existencia de varios puertos
port = 0  # Si conoce el puerto serial ponga el string ej: port='COM1'

#################################
# Sleep De los tiempos de espera entre capturas y verificación de la comunicacion
sleep_comunicacion = 1e-1
sleep_captura = 1
#################################
# formato de imagen en el cual se guardan las imagenes
image_format = "bmp"
...
def grab_next_image_by_trigger(cam, Led, image_format="bmp"):
    """
        ...
        if CHOSEN_TRIGGER == TriggerType.SOFTWARE:
            ...
            try:
                ...
                if image_result.IsIncomplete():
                    ...
                    filename = "Acquisition_" + Led[2] + "." + image_format
...
puerto = Fun_Ad.Serial_Port_Select(port=port)
...
for i, cam in enumerate(cam_list):
    ...
    for Led in Lista_Leds:
        ...
        try:
            ...
            if comunicacion.inWaiting() == 1:
                Check = comunicacion.read()
                grab_next_image_by_trigger(cam, Led, image_format=image_format)

            if Check == b"O":
                print("Comando W Aceptado")

            time.sleep(sleep_captura)
