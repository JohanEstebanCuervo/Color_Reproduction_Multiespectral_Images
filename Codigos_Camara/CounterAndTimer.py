...
def setup_counter_and_timer(nodemap):
    ...
    try:
        ...
        node_counter_selector = PySpin.CEnumerationPtr(
            nodemap.GetNode("CounterSelector")
        )
        ...
        if not PySpin.IsAvailable(entry_counter_0) or not PySpin.IsReadable(
            entry_counter_0
        ):
        ...
        node_counter_event_source = PySpin.CEnumerationPtr(
            nodemap.GetNode("CounterEventSource")
        )
        if not PySpin.IsAvailable(node_counter_event_source) or not PySpin.IsWritable(
            node_counter_event_source
        ):
            print(
        ...
        entry_counter_event_source_mhz_tick = node_counter_event_source.GetEntryByName(
            "MHzTick"
        )
        if not PySpin.IsAvailable(
            entry_counter_event_source_mhz_tick
        ...
        if not PySpin.IsAvailable(node_counter_duration) or not PySpin.IsWritable(
            node_counter_duration
        ):
        ...
        if not PySpin.IsAvailable(node_counter_delay) or not PySpin.IsWritable(
            node_counter_delay
        ):
        ...
        duty_cycle = (
            float(node_counter_duration.GetValue())
            / (float(node_counter_duration.GetValue() + node_counter_delay.GetValue()))
            * 100
        )
        ...
        pulse_rate = 1000000 / float(
            node_counter_duration.GetValue() + node_counter_delay.GetValue()
        )

        print("\nThe pulse rate has been set to {} Hz".format(pulse_rate))

        # Set Counter Trigger Source to Frame Trigger Wait
        node_counter_trigger_source = PySpin.CEnumerationPtr(
            nodemap.GetNode("CounterTriggerSource")
        )
        if not PySpin.IsAvailable(node_counter_trigger_source) or not PySpin.IsWritable(
            node_counter_trigger_source
        ):
            print(
        ...
        entry_counter_trigger_source_ftw = node_counter_trigger_source.GetEntryByName(
            "FrameTriggerWait"
        )
        if not PySpin.IsAvailable(
            entry_counter_trigger_source_ftw
        ...
        node_counter_trigger_activation = PySpin.CEnumerationPtr(
            nodemap.GetNode("CounterTriggerActivation")
        )
        if not PySpin.IsAvailable(
            node_counter_trigger_activation
        ) or not PySpin.IsWritable(node_counter_trigger_activation):
            print(
                "\nUnable to set Counter Trigger Activation (enumeration retrieval). Aborting...\n"
            )
            return False

        entry_counter_trigger_source_lh = (
            node_counter_trigger_activation.GetEntryByName("LevelHigh")
        )
        if not PySpin.IsAvailable(
            entry_counter_trigger_source_lh
...
def configure_digital_io(nodemap):
    ...
    try:
        ...
        if not PySpin.IsAvailable(node_device_name) or not PySpin.IsReadable(
            node_device_name
        ):
        ...
        if not PySpin.IsAvailable(node_line_selector) or not PySpin.IsWritable(
            node_line_selector
        ):
            print(
        ...
        if camera_family_bfs in camera_model:
            ...
            if not PySpin.IsAvailable(
                entry_line_selector_line_1
            ) or not PySpin.IsReadable(entry_line_selector_line_1):
            ...
            ) or not PySpin.IsReadable(entry_line_selector_line_2):
                print("\nUnable to set Line Selector (entry retrieval). Aborting...\n")
                return False
            ...
            if not PySpin.IsAvailable(node_line_mode) or not PySpin.IsWritable(
                node_line_mode
            ):
                print(
            ...
            if not PySpin.IsAvailable(entry_line_mode_output) or not PySpin.IsReadable(
                entry_line_mode_output
            ):
        ...
        if not PySpin.IsAvailable(node_line_source) or not PySpin.IsWritable(
            node_line_source
        ):
            print("\nUnable to set Line Source (enumeration retrieval). Aborting...\n")
            return False

        entry_line_source_counter_0_active = node_line_source.GetEntryByName(
            "Counter0Active"
        )
        if not PySpin.IsAvailable(
        ...
        if camera_family_bfs in camera_model:
            ...
            if not PySpin.IsAvailable(node_voltage_enable) or not PySpin.IsWritable(
                node_voltage_enable
            ):
                print(
...
def configure_exposure_and_trigger(nodemap):
    ...
    try:
        ...
        if not PySpin.IsAvailable(node_trigger_mode) or not PySpin.IsWritable(
            node_trigger_mode
        ):
        ...
        if not PySpin.IsAvailable(entry_trigger_mode_off) or not PySpin.IsReadable(
            entry_trigger_mode_off
        ):
            print(
        ...
        if not PySpin.IsAvailable(node_trigger_source) or not PySpin.IsWritable(
            node_trigger_source
        ):
            print(
        ...
        entry_trigger_source_counter_0_start = node_trigger_source.GetEntryByName(
            "Counter0Start"
        )
        if not PySpin.IsAvailable(
            entry_trigger_source_counter_0_start
        ...
        if not PySpin.IsAvailable(node_trigger_overlap) or not PySpin.IsWritable(
            node_trigger_overlap
        ):
            print(
        ...
        if not PySpin.IsAvailable(entry_trigger_overlap_ro) or not PySpin.IsReadable(
            entry_trigger_overlap_ro
        ):
        ...
        if not PySpin.IsAvailable(entry_trigger_mode_on) or not PySpin.IsReadable(
            entry_trigger_mode_on
        ):
            print(
...
def acquire_images(cam, nodemap, nodemap_tldevice):
    ...
    try:
        ...
        if not PySpin.IsAvailable(node_acquisition_mode) or not PySpin.IsWritable(
            ...
            print(
                "Unable to set acquisition mode to continuous (enumeration retrieval). Aborting...\n"
            )
            return False

        entry_acquisition_mode_continuous = node_acquisition_mode.GetEntryByName(
            "Continuous"
        )
        if not PySpin.IsAvailable(
            entry_acquisition_mode_continuous
            ...
            try:
                ...
                if image_result.IsIncomplete():
                    ...
                    if device_serial_number:
                        filename = "CounterAndTimer-{}-{}.jpg".format(
    ...
    *** NOTES ***
    ...
    try:
        ...
        if not PySpin.IsAvailable(node_trigger_mode) or not PySpin.IsWritable(
            ...
            print(
                "Unable to disable trigger mode (node retrieval). Non-fatal error...\n"
            )
        ...
        if not PySpin.IsAvailable(entry_trigger_mode_off) or not PySpin.IsReadable(
            ...
            print(
                "Unable to disable trigger mode (enum entry retrieval). Non-fatal error...\n"
            )
