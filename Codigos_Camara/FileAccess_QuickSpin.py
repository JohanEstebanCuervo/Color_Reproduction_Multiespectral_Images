        ...
        for image_count in range(5):
            try:
                ...
                if result_image.IsIncomplete():
                    print(
                        "Imgae incomplete with image status %s"
                        % result_image.GetImageStatus()
                    )
                else:
                    print(
                        "Grabbed image %s" % str(image_count)
                        + ", width = %s" % str(result_image.GetWidth())
...
class FileAccess:
    ...
    def upload_image(cam, verbose=False):
        ...
        try:
            ...
            if (
                cam.FileSelector.GetAccessMode() == PySpin.NA
                or cam.FileSelector.GetAccessMode() == PySpin.NI
            ):
                print("File selector not supported on device!")
                return False

            # Apply small pixel format
            if ImageAcquisitionUtil.check_node_readable(
                cam.PixelFormat.GetEntry(PySpin.PixelFormat_Mono8)
            ):
            ...
            if (
                cam.DeviceSerialNumber.GetAccessMode() == PySpin.RW
                or cam.DeviceSerialNumber.GetAccessMode() == PySpin.RO
            ):
            ...
            for entry in selector_list:
                ...
                if node.GetSymbolic() == "UserFile1":
                    ...
                    if (
                        cam.FileAccessLength.GetValue()
                        < cam.FileAccessBuffer.GetLength()
                    ):
                        try:
                            cam.FileAccessLength.SetValue(
                                cam.FileAccessBuffer.GetLength()
                            )
                        except PySpin.SpinnakerException as ex:
                            print(
                                "Unable to set FileAccessLength to FileAccessBuffer length: %s"
                                % ex
                            )
                    ...
                    write_iterations = (
                        total_bytes_to_write // intermediate_buffer_size
                    ) + (
                        0
                        if ((total_bytes_to_write % intermediate_buffer_size) == 0)
                        else 1
                    )
                    ...
                    for i in range(len(split_data)):
                        ...
                        if verbose:
                            print(
                                "File Access Offset: %s"
                                % cam.FileAccessOffset.GetValue()
                            )
                        ...
                        if verbose:
                            ...
                            print(
                                "Progress: %s" % int((i * 100 / write_iterations)) + "%"
                            )
        ...
        try:
            ...
            for entry in selector_list:
                ...
                if node.GetSymbolic() == "UserFile1":
                    ...
                    intermediate_buffer_size = cam.FileAccessLength.GetValue()
                    read_iterations = (
                        total_bytes_to_read // intermediate_buffer_size
                    ) + (
                        0
                        if ((total_bytes_to_read % intermediate_buffer_size) == 0)
                        else 1
                    )
                    ...
                    for i in range(read_iterations):
                        ...
                        if verbose:
                            print(
                                "Bytes read: %s of %s"
                                % (total_size_read, total_bytes_to_read)
                            )
                            print("Progress: (%s//%s)" % (i, read_iterations))
                        else:
                            print(
                                "Progress: %s" % int((i * 100 / read_iterations)) + "%"
                            )
                    ...
                    if ImageAcquisitionUtil.check_node_readable(
                        ...
                        cam.PixelFormat.SetValue(PySpin.PixelFormat_Mono8)
                    elif ImageAcquisitionUtil.check_node_readable(
                        cam.PixelFormat.GetEntry(PySpin.PixelFormat_BayerGB8)
                    ):
                        # Use Bayer8 if Mono8 is not available
                        cam.PixelFormat.SetValue(PySpin.PixelFormat_BayerGB8)
                    else:
                    ...
                    image = PySpin.Image.Create(
                        width, height, offset_x, offset_y, pixel_format, image_data
                    )
...
def main():
    ...
    parser = subparsers.add_parser(
        "stop", formatter_class=argparse.RawTextHelpFormatter
    )
