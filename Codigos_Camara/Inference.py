...
NETWORK_FILE_PATH = (
    ...
    if ((CHOSEN_INFERENCE_NETWORK_TYPE) == InferenceNetworkType.CLASSIFICATION)
...
INJECTED_IMAGE_WIDTH = (
    640 if CHOSEN_INFERENCE_NETWORK_TYPE == InferenceNetworkType.CLASSIFICATION else 720
)
INJECTED_IMAGE_HEIGHT = (
    400 if CHOSEN_INFERENCE_NETWORK_TYPE == InferenceNetworkType.CLASSIFICATION else 540
)
...
def camera_delete_file(nodemap):
    ...
    print("Deleting file...")
    try:
        ptr_file_operation_selector = PySpin.CEnumerationPtr(
            nodemap.GetNode("FileOperationSelector")
        )
        if not PySpin.IsWritable(ptr_file_operation_selector):
            print("Unable to configure FileOperationSelector. Aborting...")
            return False

        ptr_file_operation_delete = PySpin.CEnumEntryPtr(
            ptr_file_operation_selector.GetEntryByName("Delete")
        )
        if not PySpin.IsReadable(ptr_file_operation_delete):
            print("Unable to configure FileOperationSelector Delete. Aborting...")
            return False

        ptr_file_operation_selector.SetIntValue(
            int(ptr_file_operation_delete.GetNumericValue())
        )

        ptr_file_operation_execute = PySpin.CCommandPtr(
            nodemap.GetNode("FileOperationExecute")
        )
        ...
        ptr_file_operation_status = PySpin.CEnumerationPtr(
            nodemap.GetNode("FileOperationStatus")
        )
        if not PySpin.IsReadable(ptr_file_operation_status):
            print("Unable to query FileOperationStatus. Aborting...")
            return False

        ptr_file_operation_status_success = PySpin.CEnumEntryPtr(
            ptr_file_operation_status.GetEntryByName("Success")
        )
        ...
        if (
            ptr_file_operation_status.GetCurrentEntry().GetNumericValue()
            != ptr_file_operation_status_success.GetNumericValue()
        ):
            print(
                "Failed to delete file! File Operation Status : %s"
                % ptr_file_operation_status.GetCurrentEntry().GetSymbolic()
            )
            return False
...
def camera_open_file(nodemap):
    ...
    try:
        ...
        ptr_file_operation_open = PySpin.CEnumEntryPtr(
            ptr_file_operation_selector.GetEntryByName("Open")
        )
        ...
        ptr_file_operation_selector.SetIntValue(
            int(ptr_file_operation_open.GetNumericValue())
        )
        ...
        ptr_file_open_mode_write = PySpin.CEnumEntryPtr(
            ptr_file_open_mode.GetEntryByName("Write")
        )
        ...
        if (
            ...
            print(
                "Failed to open file for writing! File Operation Status : %s"
                % ptr_file_operation_status.GetCurrentEntry().GetSymbolic()
            )
            return False

        # Set file upload persistence settings
        ptr_file_write_to_flash = PySpin.CBooleanPtr(
            nodemap.GetNode("FileWriteToFlash")
        )
        ...
        if not PySpin.IsReadable(ptr_file_access_length) or not PySpin.IsWritable(
            ptr_file_access_length
        ):
            print("Unable to query/configure FileAccessLength. Aborting...")
            return False

        # Attempt to set FileAccessLength to FileAccessBufferNode length to speed up the write
        ptr_file_access_buffer = PySpin.CRegisterPtr(
            nodemap.GetNode("FileAccessBuffer")
        )
        ...
        if not PySpin.IsReadable(ptr_file_access_offset) or not PySpin.IsWritable(
            ptr_file_access_offset
        ):
...
def camera_write_to_file(nodemap):
    try:
        ...
        ptr_file_operation_write = PySpin.CEnumEntryPtr(
            ptr_file_operation_selector.GetEntryByName("Write")
        )
        ...
        ptr_file_operation_selector.SetIntValue(
            int(ptr_file_operation_write.GetNumericValue())
        )
        ...
        if (
            ...
            print(
                "Failed to write to file! File Operation Status : %s"
                % ptr_file_operation_status.GetCurrentEntry().GetSymbolic()
            )
            return False
...
def camera_close_file(nodemap):
    ...
    try:
        ...
        ptr_file_operation_close = PySpin.CEnumEntryPtr(
            ptr_file_operation_selector.GetEntryByName("Close")
        )
        ...
        ptr_file_operation_selector.SetIntValue(
            int(ptr_file_operation_close.GetNumericValue())
        )
        ...
        if (
            ...
            print(
                "Failed to close the file! File Operation Status : %s"
                % ptr_file_operation_status.GetCurrentEntry().GetSymbolic()
            )
            return False
...
def upload_file_to_camera(nodemap, file_selector_entry_name, file_path):
    ...
    ptr_inference_selector_entry = PySpin.CEnumEntryPtr(
        ptr_file_selector.GetEntryByName(file_selector_entry_name)
    )
    if not PySpin.IsReadable(ptr_inference_selector_entry):
        print(
            "Unable to query FileSelector entry %s " % file_selector_entry_name
            + ". Aborting..."
        )
        return False
    ...
    print(
        "Setting FileSelector to %s " % ptr_inference_selector_entry.GetSymbolic()
        + "...\n"
    )
    ...
    if camera_delete_file(nodemap) != True:
        print(
            "Failed to delete existing file for selector entry %s"
            % ptr_inference_selector_entry.GetSymbolic()
            + ". Aborting..."
        )
        return False
    ...
    if not PySpin.IsReadable(ptr_file_access_buffer) or not PySpin.IsWritable(
        ptr_file_access_buffer
    ):
        print("Unable to query FileAccessBuffer. Aborting...")
        return False
...
def delete_file_on_camera(nodemap, file_selector_entry_name):
    ...
    if not PySpin.IsReadable(ptr_inference_selector_entry):
        print(
            "Unable to query FileSelector entry "
            + file_selector_entry_name
            + ". Aborting..."
        )
        return False
...
def disable_chunk_data(nodemap):
    ...
    try:
        ...
        if CHOSEN_INFERENCE_NETWORK_TYPE == InferenceNetworkType.DETECTION:
            ...
            if result == False:
                print(
                    "Unable to disable Inference Bounding Box chunk data. Aborting..."
                )
                return result
        else:
    ...
    try:
        ...
        if CHOSEN_INFERENCE_NETWORK_TYPE == InferenceNetworkType.DETECTION:
            ...
            for i in range(box_count):
                ...
                if (
                    box.boxType
                    == InferenceBoundingBoxType.INFERENCE_BOX_TYPE_RECTANGLE.value
                ):
                    print(
                        "\t\tBox {0}: Class {1} ({2}): - {3:.4f}% - {4} (X={5} Y={6} W={7} H={8})".format(
                            i + 1,
                            box.classId,
                            LABEL_DETECTION[box.classId]
                            if box.classId < len(LABEL_DETECTION)
                            else "N/A",
                            box.confidence * 100,
                            "Rectangle",
                    ...
                    box.boxType
                    == InferenceBoundingBoxType.INFERENCE_BOX_TYPE_CIRCLE.value
                ):
                    print(
                        "\t\tBox {0}: Class {1} ({2}): - {3:.4f}% - {4} (X={5} Y={6} R={7})".format(
                            i + 1,
                            box.classId,
                            LABEL_DETECTION[box.classId]
                            if box.classId < len(LABEL_DETECTION)
                    ...
                    box.boxType
                    == InferenceBoundingBoxType.INFERENCE_BOX_TYPE_ROTATED_RECTANGLE.value
                ):
                    print(
                        "\t\tBox {0}: Class {1} ({2}): - {3:.4f}% - {4} (X1={5} Y1={6} X2={7} Y2={8} angle={9})".format(
                            i + 1,
                            box.classId,
                            LABEL_DETECTION[box.classId]
                            if box.classId < len(LABEL_DETECTION)
            ...
            print(
                " (%s)" % LABEL_CLASSIFICATION[inference_result]
                if inference_result < len(LABEL_CLASSIFICATION)
                else "N/A"
            )
...
def configure_trigger(nodemap):
    ...
    try:
        ...
        ptr_trigger_selector = PySpin.CEnumerationPtr(
        ...
        ptr_frame_start = PySpin.CEnumEntryPtr(
            ptr_trigger_selector.GetEntryByName("FrameStart")
        )
        ...
        ptr_inference_ready = PySpin.CEnumEntryPtr(
            ptr_trigger_source.GetEntryByName("InferenceReady")
        )
...
def configure_inference(nodemap, is_enabled):
    ...
    try:
        if is_enabled:
            ptr_inference_network_type_selector = PySpin.CEnumerationPtr(
                nodemap.GetNode("InferenceNetworkTypeSelector")
            )
            ...
            ptr_inference_network_type = PySpin.CEnumEntryPtr(
                ptr_inference_network_type_selector.GetEntryByName(network_type_string)
            )
            if not PySpin.IsReadable(ptr_inference_network_type):
                print(
                    "Unable to set inference network type to %s" % network_type_string
                    + " (entry retrieval). Aborting..."
                )
                return False

            inference_network_value = ptr_inference_network_type.GetNumericValue()
            ptr_inference_network_type_selector.SetIntValue(
                int(inference_network_value)
            )
...
def configure_test_pattern(nodemap, is_enabled):
    ...
    try:
        ...
        ptr_test_pattern_generator_selector = PySpin.CEnumerationPtr(
            nodemap.GetNode("TestPatternGeneratorSelector")
        )
        ...
        if is_enabled:
            ptr_test_pattern_generator_pipeline_start = PySpin.CEnumEntryPtr(
                ptr_test_pattern_generator_selector.GetEntryByName("PipelineStart")
            )
            if not PySpin.IsReadable(ptr_test_pattern_generator_pipeline_start):
                print(
                    "Unable to query TestPatternGeneratorSelector PipelineStart. Aborting..."
                )
                return False

            ptr_test_pattern_generator_selector.SetIntValue(
                int(ptr_test_pattern_generator_pipeline_start.GetNumericValue())
            )
            print(
                "TestPatternGeneratorSelector set to "
                + ptr_test_pattern_generator_pipeline_start.GetSymbolic()
                + "..."
            )

        else:
            ptr_test_pattern_generator_sensor = PySpin.CEnumEntryPtr(
                ptr_test_pattern_generator_selector.GetEntryByName("Sensor")
            )
            if not PySpin.IsReadable(ptr_test_pattern_generator_sensor):
                print(
                    "Unable to query TestPatternGeneratorSelector Sensor. Aborting..."
                )
                return False

            ptr_test_pattern_generator_selector.SetIntValue(
                int(ptr_test_pattern_generator_sensor.GetNumericValue())
            )
            print(
                "TestPatternGeneratorSelector set to "
                + ptr_test_pattern_generator_sensor.GetSymbolic()
                + "..."
            )
        ...
        if is_enabled:
            ptr_injected_image = PySpin.CEnumEntryPtr(
                ptr_test_pattern.GetEntryByName("InjectedImage")
            )
            ...
            ptr_test_pattern_off = PySpin.CEnumEntryPtr(
                ptr_test_pattern.GetEntryByName("Off")
            )
        ...
        if is_enabled:
            ...
            ptr_injected_width.SetValue(
                INJECTED_IMAGE_WIDTH if is_enabled else ptr_injected_width.GetMax()
            )
            ...
            ptr_injected_height.SetValue(
                INJECTED_IMAGE_HEIGHT if is_enabled else ptr_injected_height.GetMax()
            )
...
def acquire_images(cam, nodemap, nodemap_tldevice):
    ...
    try:
        ...
        ptr_acquisition_mode = PySpin.CEnumerationPtr(
        ...
        ptr_acquisition_mode_continuous = PySpin.CEnumEntryPtr(
