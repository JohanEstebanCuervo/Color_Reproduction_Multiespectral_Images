...
for grupo in entrenamiento:
    ...
    lista_RGB = fun.RGB_IN_mean(im_RGB, mascaras)*255
    lista_RGB = np.array(lista_RGB,dtype='uint8')
    if grupo == entrenamiento[0]:
        Datos_entrenamiento=np.concatenate((lista_RGB,color_check),axis=1)
    else:
        dato_im = np.concatenate((lista_RGB,color_check),axis=1)
        Datos_entrenamiento=np.concatenate((Datos_entrenamiento,dato_im))
        

df = pd.DataFrame(Datos_entrenamiento)
df.to_csv('Datos_entrenamiento_medias.csv',header=None,index=False)
...
for grupo in test:
    ...
    lista_RGB = np.array(lista_RGB,dtype='uint8')
    if grupo == test[0]:
        Datos_test=np.concatenate((lista_RGB,color_check),axis=1)
    else:
        dato_im = np.concatenate((lista_RGB,color_check),axis=1)
...
df.to_csv('Datos_test_medias.csv', header=None,index=False)