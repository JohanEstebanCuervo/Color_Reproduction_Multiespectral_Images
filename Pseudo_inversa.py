...
delta_Pseudoinversa=[]
for i in range(24):
    deltae= np.sqrt((color_check[i,0]-Color_parche_Irecons[i,0])**2+(color_check[i,1]-Color_parche_Irecons[i,1])**2+(color_check[i,2]-Color_parche_Irecons[i,2])**2);
    delta_Pseudoinversa=np.append(delta_Pseudoinversa,deltae)

error_manuel= [17.5314,13.5606,4.5679,8.4946,9.6954,4.7560,5.4009,11.7933,8.4918,10.5506,6.1800,10.2030,5.3575,
               18.1530,10.8929,10.6894,10.5657,12.2930,8.3776,13.2392,8.4955,12.8416,7.2840,14.3346]

Media1= np.mean(error_manuel)
Media2= np.mean(delta_Pseudoinversa)
