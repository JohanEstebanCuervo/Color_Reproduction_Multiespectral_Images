...
def imwrite(titulo, imagen, InColorSpace="RGB"):
    ...
    if InColorSpace == "RGB":
        ...
        print("Espacio de color no creado.")
        return
...
def Lab2sRGB(imagen_CIE):
    ...
    imagen_CIELAB = np.where(
        imagen_CIELAB > 0.206893, imagen_CIELAB**3, (imagen_CIELAB - 16 / 116) / 7.787
    )
...
def ext_mascaras(carpeta, lista):
    ...
    for nombre in sorted(lista):
        a = cv2.cvtColor(
            cv2.imread(carpeta + "/" + nombre), cv2.COLOR_BGR2GRAY
        )  # lectura de imagen, transformacion a escala de grises
        [T, mascara] = cv2.threshold(
            a, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU
        )  # binarizar mascara con algotirmo de OTSU
...
def Read_Multiespectral_imag(carpeta, lista):
    ...
    for nombre in sorted(lista):
        imagen = cv2.cvtColor(
            cv2.imread(carpeta + "/" + nombre), cv2.COLOR_BGR2GRAY
        )  # cargamos imagenes multiespectrales en escala de grises
        imagenespatron = np.concatenate(
            (imagenespatron, np.squeeze(np.reshape(imagen, (1, -1)))), axis=0
        )  # se convierte la imagen en una columna y se concatena con las demas del espectro
    shape_imag = np.shape(imagen)
    imagenespatron = imagenespatron.reshape(
        len(lista), -1
    )  # se redimensiona a  Filas * N imagenes multiespectrales filas de pixeles de las imagenes espectrales
...
def Pesos_ecualizacion(
    imagenes_patron, mascara, valor_ideal=243, shape_imag=(480, 640)
):
    promedios = []
...
def ecualizacion(prom, parche, brillo_parche):
    pesos = np.divide(
        brillo_parche * np.ones((1, len(prom[0]))), prom[parche - 1, :]
    )  # pesos para cada espectro utilizando de referencia un parche neutral y su color ideal
    ecualizado = prom * pesos  # se ecualiza los promedios
    ecualizado[
        np.where(ecualizado > 255)
    ] = 255  # los valores saturados se igualan a 255
...
#%%
def ReproduccionCie1931(
    imagenes_patron, shape_imag=(480, 640, 3), selec_imagenes="All", OutColorSpace="RGB"
):
    ...
    #% Coeficientes
    Coef = (
        CIE1931[selec_imagenes, 1:] * (np.ones((3, 1)) * D65[selec_imagenes, 1].T).T
    ).T
...
def ReproduccionCie19312(
    imagenes_patron, Pesos_ecu, shape_imag=(480, 640, 3), selec_imagenes="All"
):
...
def mejor_combinacion(
    ...
    for i, Comb in enumerate(subset):
        ...
        image_recons = ReproduccionCie1931(
            imagenes_patron, selec_imagenes=Comb, OutColorSpace=OutColorSpace
        )
...
def combinaciones_lista_errores(
    imagenes_patron,
    mascaras,
    color_check,
    Cant_Image,
