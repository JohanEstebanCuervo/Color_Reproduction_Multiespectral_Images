...
_, _, _, _,_ = fun.Imagenes_Camara(carpeta1 ,lista_patron, mascaras, color_check)
...
imagenes_patron = imagenes_patron[:-2]

xyz = np.dot(Coef,imagenes_patron).T/13
maximos=np.max(xyz,axis=0)
xyz = np.divide(xyz,np.max(xyz,axis=0))

im_Y=np.reshape(xyz[:,1],shape_imag)
...
im_RGB=np.reshape(rgb,np.append(shape_imag,3))
...
mte2rgb= np.dot(XYZ2RGB,Coef2)*k/13

rgb2=fun.recorte(np.dot(mte2rgb,imagenes_patron))
im_RGB2=np.reshape(rgb2.T,np.append(shape_imag,3))
...
im_RGB3,Ccm_lineal = fun.CCM_Linear(im_RGB2, color_RGB_pixel_ideal, mascaras)

fun.imshow('Imagen mejorada mediante con lineal', im_RGB3)

#%% CMM TRANSFORM Compund 

im_RGB4,Ccm_Compound = fun.CCM_Compound(im_RGB2, color_RGB_pixel_ideal, mascaras)

fun.imshow('Imagen mejorada mediante ccm compund', im_RGB4)

#%% CMM TRANSFORM logarithm

im_RGB5,Ccm_Logarithm = fun.CCM_Logarithm(im_RGB2, color_RGB_pixel_ideal, mascaras)
...
im_RGB7,Ccm_Polynomial, r2 = fun.CCM_Polynomial(im_RGB2, color_RGB_pixel_ideal, mascaras)
...
rgb= np.reshape(im_RGB2,(-1,3))*255
